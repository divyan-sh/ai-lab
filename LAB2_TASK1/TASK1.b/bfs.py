...
def generateMap2d_obstacle(size_):
    size_x, size_y = size_
    map2d = np.zeros((size_x, size_y))

    # Adding small random obstacles
    num_small_obstacles = int(size_x * size_y * 0.1)  # 10% of map size
    for _ in range(num_small_obstacles):
        x, y = np.random.randint(0, size_x), np.random.randint(0, size_y)
        map2d[x, y] = -1

    # Add special obstacle (rotated-H)
    ...
    while True:
        ...
        goalp = [np.random.randint(size_x // 2 + 4, size_x - 3), np.random.randint(5, size_y - 5)]
...
def get_neighbors(node, map2d):
    ...
    for dx, dy in directions:
        nx, ny = x + dx, y + dy
        if 0 <= nx < map2d.shape[0] and 0 <= ny < map2d.shape[1] and map2d[nx, ny] >= 0:
...
# Plot the path
plotMap(map_with_obstacle, [[p[1] for p in path], [p[0] for p in path]])
